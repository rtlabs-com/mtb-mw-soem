...
int ecx_setupnic(ecx_portt *port, const char *ifname, int secondary)
{
   ...
   port->getindex_mutex = os_mutex_create();
   port->tx_mutex = os_mutex_create();
   port->rx_mutex = os_mutex_create();
...
int ecx_closenic(ecx_portt *port)
{
   ...
   {
      /* tbd -- dynamically shut down network, for now always on once setup */
   }
...
uint8 ecx_getindex(ecx_portt *port)
{
   ...
   os_mutex_lock(port->getindex_mutex);
   ...
   os_mutex_unlock(port->getindex_mutex);
